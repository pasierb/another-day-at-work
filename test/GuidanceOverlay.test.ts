...
describe('guidance presentation',()=>{
 it('names every required concept concisely',()=>{const copy=Object.values(GUIDANCE_COPY).join(' ');for(const term of ['no winning','Prompt','Plan','approve','Sticky alerts','Stamina','START WORKING'])assert.match(copy,new RegExp(term,'i'));});
