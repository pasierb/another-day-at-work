...
test('inline choice resolves once and results restart',async({page})=>{await start(page);await expect.poll(async()=>(await snap(page))?.activeInterruptionIds.length).toBeGreaterThan(0);const target=await page.evaluate(()=>{const s=(window as any).__PHASER_GAME__.scene.getScene('Workstation'),card=s.stickyLayer.list[0],button=card.list.find((child:any)=>child.input);return card.getWorldTransformMatrix().transformPoint(button.x+button.width/2,button.y+button.height/2);});await point(page,target.x,target.y);await expect.poll(async()=>(await snap(page))?.resolvedInterruptionIds.length).toBeGreaterThan(0);await page.evaluate(()=>{const s=(window as any).__PHASER_GAME__.scene.getScene('Workstation');s.workday.mutateResource('stamina',-100);});await expect.poll(async()=>(await snap(page))?.scene).toBe('results');await point(page,640,630);await expect.poll(async()=>(await snap(page))?.scene).toBe('workstation');});
...
test('notification cards fit their contents and six-card layout',async({page})=>{
 await start(page);
 const problems=await page.evaluate(()=>{
  ...
  for(const definition of s.interruptions.definitions.values()){
   ...
   s.buildSticky({...definition,severity:'critical',activationSequence:1,ageGameMinutes:120,status:'active'},680,352);
   const card=s.stickyLayer.list[0];
   if(card.height>250)failures.push(`${definition.id}: card exceeds row`);
   ...
   if(card.list.filter((child:any)=>child.input).length!==definition.choices.length)failures.push(`${definition.id}: missing choice`);
  }
  s.stickyLayer.removeAll(true);
