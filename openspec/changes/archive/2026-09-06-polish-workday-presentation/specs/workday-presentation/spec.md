...
The polished workstation and results presentation SHALL preserve the fixed logical composition and remain readable and operable at the project's minimum supported touch-sized viewport.

#### Scenario: Minimum supported viewport is used
- **WHEN** the workstation or results view is displayed at the minimum supported viewport
- **THEN** essential status, severity, enabled controls, decision choices, result explanation, and mute state remain readable and non-overlapping
