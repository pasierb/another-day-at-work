...
export function installResultsDiagnostics (runId: number, completedTaskIds: readonly string[], resolvedInterruptionIds: readonly string[]): () => void {
    if (!playtestDiagnosticsEnabled()) return () => {};
    const getSnapshot=():PlaytestMilestones=>Object.freeze({runId,scene:'results',booted:true,clock:'17:00',elapsedGameMs:28_800_000,paused:false,pauseReasons:Object.freeze([]),activeInterruptionIds:Object.freeze([]),resolvedInterruptionIds:Object.freeze([...resolvedInterruptionIds]),completedTaskIds:Object.freeze([...completedTaskIds]),codingRequested:false,aiPhase:'available',aiProgress:0,cycleId:null,batchId:null,reviewAvailable:false,guidanceVisible:false,terminal:true});
