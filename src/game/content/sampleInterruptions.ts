...
const production:InterruptionDefinition={...PRODUCTION_INTERRUPTION,incidentControl:'player-fixable',escalationStages:[{...PRODUCTION_INTERRUPTION.escalationStages![0]!,delayGameMs:20*60_000,postponeGameMs:10*60_000}],choices:PRODUCTION_INTERRUPTION.choices.map(item=>item.id==='hotfix'?{...item,outcomeCertainty:'uncertain',effects:[...item.effects,{type:'delayed',id:'hotfix-regression',delayGameMs:30*60_000,scheduledFeedback:'Regression monitoring scheduled.',concludedFeedback:'Regression check concluded.',effects:[]}]}:item)};
