...
export interface EventSchedulerConfig { readonly minimumSpawnIntervalMs: number; readonly maximumSpawnIntervalMs: number; readonly activeLimit: number; readonly endGameMs: number;readonly pacingProfile?:PacingProfile }
export interface SchedulerDefinition { readonly id:string;readonly category?:InterruptionCategory; readonly baseWeight?:number; readonly debtWeightModifier?:import('./effects').TechnicalDebtModifier;readonly selectionWeightModifier?:SelectionWeightModifier;readonly eligibility?:EligibilityPredicate;readonly deduplicationKey?:string;readonly cooldownGameMs?:number }
...
export class EventScheduler {
    ...
    synchronize (gameMs: number, activate: (candidate: ScheduledCandidate) => boolean, contextOrDebt:InterruptionRunContext|number=0): readonly ScheduledCandidate[] {
        ...
        while (this.nextSpawnGameMs !== null && this.nextSpawnGameMs <= gameMs && this.nextSpawnGameMs < this.config.endGameMs) {
            ...
            if(guard&&this.activeIds.size+this.pending.length>=guard.floodPressure){this.floodDeferrals++;this.nextSpawnGameMs=Math.min(this.config.endGameMs-1,due+guard.floodDeferralGameMs);break;}
    ...
    reset ():void{this.random.reset();this.pending.length=0;this.activeIds.clear();this.createdIds.clear();this.sequence=0;this.lastMeaningfulGameMs=0;this.currentGameMs=0;this.floodDeferrals=0;this.emptyQuietEvaluations=0;this.nextSpawnGameMs=this.drawInterval(0);}
    ...
    private drawInterval(from:number):number|null{if(from>=this.config.endGameMs)return null;const band=this.config.pacingProfile?activePacingBand(this.config.pacingProfile,Math.min(from,this.config.endGameMs)):undefined;const minimum=band?.minimumSpawnIntervalMs??this.config.minimumSpawnIntervalMs,maximum=band?.maximumSpawnIntervalMs??this.config.maximumSpawnIntervalMs;const span=maximum-minimum;const next=from+minimum+Math.floor(this.random.next()*(span+1));return next>=this.config.endGameMs?null:next;}
